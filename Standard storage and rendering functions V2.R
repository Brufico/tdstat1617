...
# -----------------------
# set a "textfiles" sub directory that will contain all text patterns
# name of each text pattern files = namefun_txt.Rmd ==> move the name "textfiles" to a default setting
#
#
#  At the beginning: read all textfiles and store the corresponding lines under the function name
# dir("textfiles")
# function version 2
allfilelist <- dir("textfiles")
iflist <- grep("_text.Rmd",allfilelist)
filelist <- allfilelist[iflist] # get the correct list of files
funlist <- gsub("_text.Rmd", "", filelist) #get the list of function names . match ?
